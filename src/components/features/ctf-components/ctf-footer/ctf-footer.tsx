...
export const CtfFooter = (props: FooterFieldsFragment) => {
  ...
  const renderMenuGroupLinks = (menuGroup, listClassName) => {
    return menuGroup?.items?.map(menuItem => {
      const href = menuItem.linkUrl;
      const linkText = menuItem.linkHeading;
      return (
        <li
          key={menuItem.sys.id}
    ...
    <>
      <Container {...containerProps} maxWidth={false} className={classes.footerContainer}>
        <footer className={classes.footer}>
          {footerContent?.footerLinksCollection?.items?.length && (
            <nav role="navigation" className={classes.menuWrapper}>
              {footerContent.footerLinksCollection.items.map(
                menuItem =>
                  menuItem && (
                      ...
                      <ul className={classes.menu}>
                        <li>
                          <p
                            ...
                            {...inspectorMode({
                              entryId: menuItem.sys.id,
                              fieldId: 'subNavigationItemTitle',
                              locale,
                            })}
                          >
                            {menuItem.subNavigationItemTitle}
                          </p>
                          {menuItem.secondaryLinksCollection && (
